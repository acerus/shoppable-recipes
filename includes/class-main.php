...
class Main {
	...
	public function requirements() {
		...
		if ( ! class_exists( 'DOMDocument' ) ) {
			...
			$this->notifications->add(
				'dom_document_error',
				false,
				__( 'Для правильной работы плагина <strong>Whisk Recipe Widgets</strong> необходимо расширение <strong>DOMDocument</strong>. Обратитесь в техподдержку вашего хостинга или к вашему системному администратору.', 'whisk-recipe-widgets' ),
				[
					'scope'         => 'user',
	...
	public function add_category_meta_box( \WP_Term $term ) {
		...
		?>
		<h2><?php _e( 'Pulse Mail.ru', 'whisk-recipe-widgets' ); ?></h2>
		<table class="form-table" role="presentation">
			<tr class="form-field">
				<th scope="row">
					<label for="<?php echo esc_attr( $this->slug ); ?>_exclude"><?php _e( 'Exclude From Feed', 'whisk-recipe-widgets' ); ?></label>
				</th>
				<td>
	...
	public function add_post_meta_box() {
		add_meta_box(
			$this->slug,
			__( 'Pulse Mail.ru', 'whisk-recipe-widgets' ),
	...
	public function render_meta_box( $post ) {
		...
		<label for="<?php echo esc_attr( $this->slug ); ?>_exclude" title="Включить/Исключить запись из ленты">
			<input type="checkbox" value="1" name="<?php echo esc_attr( $this->slug ); ?>_exclude" id="<?php echo esc_attr( $this->slug ); ?>_exclude" <?php checked( $exclude, true ); ?>> <?php _e( 'Exclude From Feed', 'whisk-recipe-widgets' ); ?>
		</label>
		<?php
	}
	/**
